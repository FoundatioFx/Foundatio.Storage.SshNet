# Agent Guidelines for Foundatio.Storage.SshNet
...
```bash
# Build
dotnet build Foundatio.Storage.SshNet.slnx

# Test
dotnet test Foundatio.Storage.SshNet.slnx

# Format code
dotnet format Foundatio.Storage.SshNet.slnx
```
...
```text
src
└── Foundatio.Storage.SshNet                    # SSH-based file storage implementation
    ├── Storage                                 # IFileStorage implementations for SSH
    └── Extensions                              # Extension methods for DI
tests
└── Foundatio.Storage.SshNet.Tests              # Unit and integration tests
```
...
1. **Builds successfully**: `dotnet build Foundatio.Storage.SshNet.slnx` exits with code 0
2. **All tests pass**: `dotnet test Foundatio.Storage.SshNet.slnx` shows no failures
...
- Mirror the main code structure (e.g., `Storage/` tests for `src/Foundatio.Storage.SshNet/Storage/`)
...
# Specific test file
dotnet test --filter "FullyQualifiedName~SshNetStorageTests"
